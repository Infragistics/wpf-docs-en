...
The default value of link:{ApiPlatform}controls.charts.xamdatachart.v{ProductVersion}~infragistics.controls.charts.axis~labelsettings.html[LabelSettings] property of an link:{ApiPlatform}controls.charts.xamdatachart.v{ProductVersion}~infragistics.controls.charts.axis.html[Axis] is null and this property must be instantiated with the link:{ApiPlatform}controls.charts.xamdatachart.v{ProductVersion}~infragistics.controls.charts.axislabelsettings.html[AxisLabelSettings] constructor before accessing properties of axis labels.
...
Starting from the 2011 Volume 1 release, the link:{ApiPlatform}olap.flatdata.v{ProductVersion}~infragistics.olap.flatdata.flatdatasource.html[FlatDataSource] requires unique names for hierarchies on which to base the slice of data to be computed for the link:{ApiPlatform}controls.grids.xampivotgrid.v{ProductVersion}~infragistics.controls.grids.xampivotgrid.html[xamPivotGrid] control instead of specifying only the dimension name because there was just one hierarchy per dimension in prior releases.
...
This change affects the developers that have implemented their custom data provider for the xamPivotGrid control. The following methods of the link:{ApiPlatform}olap.v{ProductVersion}~infragistics.olap.dataproviderbase.html[DataProviderBase] class have changed:
