...
This topic is designed to get you up and running with the link:{ApiPlatform}dragdrop.v{ProductVersion}.html[Infragistics Drag and Drop Framework]™ library as quickly as possible by describing the basic steps required for implementing the drag and drop functionality within your {PlatformName} application.
...
[start=8]
. Add drag and drop functionality to the chart. This is done by adding the link:{ApiPlatform}dragdrop.v{ProductVersion}~infragistics.dragdrop.dragsource.html[DragSource] object and setting the following properties:

** link:{ApiPlatform}dragdrop.v{ProductVersion}~infragistics.dragdrop.dragsource~isdraggable.html[IsDraggable] – True
** link:{ApiPlatform}dragdrop.v{ProductVersion}~infragistics.dragdrop.dragsource~dragchannels.html[DragChannels] – ChannelA (this specifies the areas where this control can be dropped to. For more information, see link:drag-and-drop-framework-set-drag-and-drop-channels.html[Set Drag and Drop Channels].)
** link:{ApiPlatform}dragdrop.v{ProductVersion}~infragistics.dragdrop.dragsource~drop_ev.html[Drop] – DragSource_Drop
...
[start=9]
. Add the target StackPanel container and indicate it will be the area that the dragged control can be dropped into. This is done by adding the link:{ApiPlatform}dragdrop.v{ProductVersion}~infragistics.dragdrop.droptarget.html[DropTarget] object and setting the following properties:

** link:{ApiPlatform}dragdrop.v{ProductVersion}~infragistics.dragdrop.droptarget~isdroptarget.html[IsDropTarget] – True
** link:{ApiPlatform}dragdrop.v{ProductVersion}~infragistics.dragdrop.droptarget~dropchannels.html[DropChannels] – ChannelA (this specifies that items with DragChannels set to ChannelA can be dropped here. For more information, see link:drag-and-drop-framework-set-drag-and-drop-channels.html[Set Drag and Drop Channels].)
