...
|The `Worksheet` object’s method link:{ApiPlatform}documents.excel{ApiVersion}~infragistics.documents.excel.worksheet~getregions.html[GetRegions] allows a named reference to be specified to access an array of regions.

|Getting a Referenced Cell, Region, and Array of Regions from a Named Reference
|A reference to a cell or region can be obtained using the link:{ApiPlatform}documents.excel{ApiVersion}~infragistics.documents.excel.namedreference_members.html[NamedReference] instance that refers to a cell or region.
