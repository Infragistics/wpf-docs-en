...
A comment allows you to display hints or notes for a cell when the end user's mouse hovers over a cell. The comments display as a tooltip-like callout that contains text. The Infragistics Excel Engine allows you to add comments to a cell by setting a link:{ApiPlatform}documents.excel.v{ProductVersion}~infragistics.documents.excel.worksheetcell.html[WorksheetCell] object’s link:{ApiPlatform}documents.excel.v{ProductVersion}~infragistics.documents.excel.worksheetcell~comment.html[Comment] property.
