...
The Infragistics Excel Engine allows you to add Microsoft® Excel® formulas to a cell or group of cells in a worksheet. You can do this using the link:{ApiPlatform}documents.excel{ApiVersion}~infragistics.documents.excel.worksheetcell.html[WorksheetCell] object’s link:{ApiPlatform}documents.excel{ApiVersion}~infragistics.documents.excel.worksheetcell~applyformula.html[ApplyFormula] method or by instantiating a link:{ApiPlatform}documents.excel{ApiVersion}~infragistics.documents.excel.formula.html[Formula] object and applying it to a cell. Regardless of the manner in which you apply a formula to a cell, you can access the Formula object using the WorksheetCell object's link:{ApiPlatform}documents.excel{ApiVersion}~infragistics.documents.excel.worksheetcell~formula.html[Formula] property. If you need the value, use the cell's link:{ApiPlatform}documents.excel{ApiVersion}~infragistics.documents.excel.worksheetcell~value.html[Value] property.
