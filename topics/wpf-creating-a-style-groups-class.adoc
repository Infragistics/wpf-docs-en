...
[start=6]
. The DataPresenter class derives from link:{ApiPlatform}datapresenter{ApiVersion}~infragistics.windows.themes.datapresenterresourceset`1.html[DataPresenterResourceSet]. The link:{RootAssembly}{ApiVersion}~infragistics.windows.themes.resourceset.html[ResourceSet] that you derive from depends upon the style group (i.e., if you were creating the ResourceSet for the Editors, you would derive from EditorsResourceSet).
...
[start=8]
. A style group's class file must contain a Locator class. The Locator class derives from link:{RootAssembly}{ApiVersion}~infragistics.windows.themes.resourcesetlocator.html[ResourceSetLocator] and defines key properties such as:

** access to the link:{RootAssembly}{ApiVersion}~infragistics.windows.themes.resourcesetlocator~assembly.html[Assembly] containing the resource set
** the link:{RootAssembly}{ApiVersion}~infragistics.windows.themes.resourcesetlocator~theme.html[Theme] or name of the style
** the link:{RootAssembly}{ApiVersion}~infragistics.windows.themes.resourcesetlocator~grouping.html[Grouping], which is specific to the style group
** the link:{RootAssembly}{ApiVersion}~infragistics.windows.themes.resourcesetlocator~resourcepath.html[ResourcePath] to the embedded ResourceDictionary for the style group within the assembly
