...
The xamCarouselPanel™ control displays a number of items along a path. You can change the number of items displayed along the path by modifying the link:{RootAssembly}{ApiVersion}~infragistics.windows.controls.carouselviewsettings~itemsperpage.html[ItemsPerPage] property of the link:{RootAssembly}{ApiVersion}~infragistics.windows.controls.carouselviewsettings.html[CarouselViewSettings] object.
