...
The xamComboEditor control does not bind directly to a data source; instead, xamComboEditor uses an instance of a link:{ApiPlatform}editors.v{ProductVersion}~infragistics.windows.editors.comboboxitemsprovider.html[ComboBoxItemsProvider] object to bind to data. You can data bind the ComboBoxItemsProvider object using the same patterns you would use for any Microsoft® Windows® Presentation Foundation ItemsControl. You can set the link:{ApiPlatform}editors.v{ProductVersion}~infragistics.windows.editors.comboboxitemsprovider~itemssource.html[ItemsSource] property to a binding expression that references a collection of objects to populate the drop-down list.
