...
You should set the link:{ApiPlatform}datapresenter{ApiVersion}~infragistics.windows.datapresenter.fielditem~name.html[Name] property so that you can use it as a key to index into the Fields collection. You can also set the Label property to display text in the header above the Field.
