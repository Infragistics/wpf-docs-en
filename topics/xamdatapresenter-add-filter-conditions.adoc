...
The `RecordFilter` object represents a collection of conditions for the cell values in a field. You can define conditions by adding objects that implement the `ICondition` interface to the `RecordFilter` object's `Conditions` collection. The Infragistics.Windows.Controls namespace in the {ApiPlatform}{ApiVersion} assembly offers the following condition objects:
