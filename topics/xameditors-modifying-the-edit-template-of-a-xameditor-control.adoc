...
The xamEditor controls use two templates -- one for displaying values and one for editing values. You can modify the control template used for editing functionality by setting the link:{ApiPlatform}editors{ApiVersion}~infragistics.windows.editors.valueeditor~edittemplate.html[EditTemplate] property exposed by the xamEditor control.
...
.. Set the Value property to a binding expression that binds to the xamNumericEditor control's link:{ApiPlatform}editors{ApiVersion}~infragistics.windows.editors.valueeditor~value.html[Value] property (TemplatedParent).
