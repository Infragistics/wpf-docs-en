...
You can enable row selectors on the xamGrid by setting the link:{ApiPlatform}controls.grids.xamgrid.v{ProductVersion}~infragistics.controls.grids.rowselectorsettings.html[RowSelectorSettings] object’s link:{ApiPlatform}controls.grids.xamgrid.v{ProductVersion}~infragistics.controls.grids.visualsettingsbase~visibility.html[Visibility] property to Visible as demonstrated in the code below.
