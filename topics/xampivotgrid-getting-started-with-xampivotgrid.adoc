...
This topic demonstrates how to get started with the link:{ApiPlatform}controls.grids.xampivotgrid.v{ProductVersion}~infragistics.controls.grids.xampivotgrid.html[xamPivotGrid]™ control by providing step-by-step procedure for adding this control to a {PlatformName} application. Code examples include listings in XAML as well as procedural code.
