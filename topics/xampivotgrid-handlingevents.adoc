...
The xamPivotGrid control’s DataSource exposes only a few events of the IOlapViewModel interface and they are all related to loading the data. For a complete list refer to the IOlapViewModel interface in {ApiPlatform}Olap.{ApiVersion} assembly section of the API Reference Guide.
