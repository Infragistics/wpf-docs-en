...
After you add a link:{ApiPlatform}ribbon.v{ProductVersion}~infragistics.windows.ribbon.gallerytool.html[GalleryTool] to xamRibbon™, you will have to add link:{ApiPlatform}ribbon.v{ProductVersion}~infragistics.windows.ribbon.galleryitem.html[GalleryItems] to the GalleryTool so your end users can select an item within the GalleryTool.

The GalleryItem object is easy to use since it only has a couple of properties that you need to set. Each GalleryItem that you add to a GalleryTool must have a unique key. You can use the key to identify the selected GalleryItem in the code-behind or to add GalleryItems into a GalleryItemGroup. In addition to the link:{ApiPlatform}ribbon.v{ProductVersion}~infragistics.windows.ribbon.galleryitem~key.html[Key] property, you can also set simple properties like the link:{ApiPlatform}ribbon.v{ProductVersion}~infragistics.windows.ribbon.galleryitem~text.html[Text] property and the link:{ApiPlatform}ribbon.v{ProductVersion}~infragistics.windows.ribbon.galleryitem~image.html[Image] property to give the GalleryItem a unique appearance. You can also specify link:{ApiPlatform}ribbon.v{ProductVersion}~infragistics.windows.ribbon.galleryitemsettings.html[GalleryItemSettings] at the GalleryItem level if you want to override the GalleryItemSettings specified at the GalleryTool level.
