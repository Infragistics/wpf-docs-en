...
There is only one stipulation to adding tools to the QAT. You can only add a link:{ApiPlatform}ribbon.v{ProductVersion}~infragistics.windows.ribbon.qatplaceholdertool.html[QatPlaceholderTool] to the QAT. When you add a QatPlaceholder tool to the QAT, you can set the tool's link:{ApiPlatform}ribbon.v{ProductVersion}~infragistics.windows.ribbon.qatplaceholdertool~targetid.html[TargetID] to the ID of any tool or group currently placed in xamRibbon™ or not in xamRibbon (a tool residing in the link:{ApiPlatform}ribbon.v{ProductVersion}~infragistics.windows.ribbon.xamribbon~toolsnotinribbon.html[ToolsNotInRibbon] collection). The ToolsNotInRibbon collection was designed specifically with the QAT in mind. Since you can only place a QatPlaceholder tool on the QAT, you need a collection for tools if you don't want those tools placed anywhere else on xamRibbon.
