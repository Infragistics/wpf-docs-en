...
The xamRibbon™ Screen Tip is a beefed-up version of the standard Screen Tip. Deriving from the XamScreenTip class itself, the link:{ApiPlatform}ribbon.v{ProductVersion}~infragistics.windows.ribbon.xamribbonscreentip.html[XamRibbonScreenTip] adds additional support for Microsoft® Office® 2007 guidelines, as well as support for an image. When setting the link:{ApiPlatform}ribbon.v{ProductVersion}~infragistics.windows.ribbon.xamribbonscreentip~contentimage.html[ContentImage] property, the xamRibbon Screen Tip restricts its own width to 318 pixels, and 210 pixels when no image is specified.
