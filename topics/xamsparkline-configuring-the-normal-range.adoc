...
This is achieved through customizing the link:{ApiPlatform}controls.charts.xamsparkline.v{ProductVersion}~infragistics.controls.charts.xamsparkline~normalrangefill.html[NormalRangeFill] property of  _xamSparkline_  . The Opacity property is used, too, in order to make sure the chart is visible through the range.
