...
The following code uses the minimum of the property setting to display the normal range. The link:{ApiPlatform}controls.charts.xamsparkline.v{ProductVersion}~infragistics.controls.charts.xamsparkline~normalrangefill.html[NormalRangeFill] property is not required. It displays light-gray by default, as it is shown in the preview above, although it can be changed with desired color.
