...
If you add link:{RootAssembly}{ApiVersion}~infragistics.windows.controls.tabitemex.html[TabItemEx] objects to xamTabControl™ and the xamTabControl's link:{RootAssembly}{ApiVersion}~infragistics.windows.controls.xamtabcontrol~allowtabclosing.html[AllowTabClosing] property is set to True, an end user can close a tab by middle clicking the tab's header.
