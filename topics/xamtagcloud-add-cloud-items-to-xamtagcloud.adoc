...
Setting the link:{ApiPlatform}controls.menus.xamtagcloud.v{ProductVersion}~infragistics.controls.menus.xamtagclouditem.html[XamTagCloudItem] object’s link:{ApiPlatform}controls.menus.xamtagcloud.v{ProductVersion}~infragistics.controls.menus.xamtagclouditem~weight.html[Weight] and Content properties determines the tags in the cloud and the size of each individual tag item.
