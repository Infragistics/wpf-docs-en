...
Adding multiple layers to xamMap™ allows for a map with multiple display surfaces, such as a map with states, cities, and roads. Each link:{ApiPlatform}controls.maps.xammap{ApiVersion}~infragistics.controls.maps.maplayer.html[MapLayer] object may bind to a Shapefile or to a geo-imagery data source (*note* : at most one layer in the collection may bind to a geo-imagery data source). When these layers are combined, they stack based on the order in which they are added to the Layers collection, with the last layer added as the top-most layer.

If you want layers to display at a certain window scale, you can set the link:{ApiPlatform}controls.maps.xammap{ApiVersion}~infragistics.controls.maps.maplayer~visiblefromscale.html[VisibleFromScale] property. The link:{ApiPlatform}controls.maps.xammap{ApiVersion}~infragistics.controls.maps.xammap.html[XamMap] control displays the layer if the link:{ApiPlatform}controls.maps.xammap{ApiVersion}~infragistics.controls.maps.xammap~windowscale.html[WindowScale] is increased equal to or greater than the value of the VisibleFromScale property.
...
[start=3]
. In the code behind, add an event handler for the xamMap control's link:{ApiPlatform}controls.maps.xammap{ApiVersion}~infragistics.controls.maps.xammap~windowrectchanged_ev.html[WindowRectChanged] event. This event fires when the map window is changed (e.g. when the map's pan or zoom changes).
...
[start=4]
. Add two MapLayer objects to display the information from the Shapefiles. Since we only want the states layer to be visible at a certain scale, set link:{ApiPlatform}controls.maps.xammap{ApiVersion}~infragistics.controls.maps.maplayer~visiblefromscale.html[VisibleFromScale] property to 2 on that layer.
